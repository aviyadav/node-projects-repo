...
const ChartVisualization = ({ data, selectedPoints, onPointSelect }) => {
  console.log("Multi-Chart Visualization loaded - NEW VERSION", {
  ...
  const handleChartClick = (clickedData) => {
    if (
      clickedData &&
      clickedData.activePayload &&
      clickedData.activePayload[0]
    ) {
  ...
  if (data.length === 0) {
    return (
      <EmptyMessage>
        No data available for the current selection criteria.
        <br />
  ...
  return (
    <>
        ...
        <ChartContainer>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis dataKey="x" name="Quantity" tick={{ fontSize: 10 }} />
              <YAxis dataKey="y" name="Sales Amount" tick={{ fontSize: 10 }} />
      ...
      >
        <strong>How to use:</strong> Charts are displayed horizontally. Click on
        any data point in the Scatter, Bar, or Line charts to select it.
        Selected points will be highlighted in red across all charts and filter
        the table below. Click on a selected point again to deselect it. The Pie
        Chart shows category summaries and is not interactive.
      </div>
    </>
  );
};
